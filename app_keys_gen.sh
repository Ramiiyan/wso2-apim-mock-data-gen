...
APP_LIST_RESPONSE=$(curl -s -k -H "Authorization: Bearer $ACCESS_TOKEN" \
                       "https://$HOST:$SERVLET_PORT/api/am/devportal/v3/applications") # In APIM 4.5.0, Devportal Restful API version is v3
...
for app_id in "${APP_IDS[@]}"; do
  ...
  for key_type in "PRODUCTION" "SANDBOX"; do
    ...
    KEYS_RESPONSE=$(curl -s -k -X POST \
                        ...
                        "https://$HOST:$SERVLET_PORT/api/am/devportal/v3/applications/$app_id/generate-keys") # In APIM 4.5.0, Devportal Restful API version is v3
